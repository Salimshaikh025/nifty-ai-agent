...
<div style="text-align:center; color:#0f0; font-size:11px; padding:4px;">BUILD-FIX-v9-SLTARGET</div>
...
async function turnOn() {
    ...
    const ok = confirm("ENABLE LIVE AUTO-PILOT? Maximum trades: " + trades + ". Maximum amount: Rs./-" + amount.toFixed(2) + ". Stop Loss: " + slPct + "%. Target: " + targetPct + "%. REAL MONEY ORDERS CAN BE PLACED.");
    if (!ok) return;
    try {
        const response = await fetch("/api/autopilot", {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({enabled:true,max_trades:trades,max_amount:amount,stop_loss_percent:slPct,target_percent:targetPct})});
